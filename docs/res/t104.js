...
function initEventHandlers(canvas, q_rot, q_inc, pan_and_eye) {
	...
	var lastX = -1, lastY = -1; // Last position of the mouse for updating each time step
	var initX = -1, initY = -1; // Last position of the mouse for end spining
	var u, v;
	var action = 0;
	...
	canvas.onmousedown = function (ev) { // Mouse is pressed
		...
		if (-1 <= x && x < 1 && -1 <= y && y < 1) {
			initX = ev.clientX; initY = ev.clientY;
			lastX = x; lastY = y;
			dragging = true;
			action = ev.button + 1;
		}
