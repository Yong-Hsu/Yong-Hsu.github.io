...
window.onload = function init() {
	...
	canvas.addEventListener("click", function(event) {
		...
		var t = vec3(-1 + 2 * (event.clientX - bbox.left) / canvas.width,
					-1 + 2 * (canvas.height - event.clientY + bbox.top) / canvas.height,
					1 - 2 * (index+1) / maxNumVertices);
		...
		if (drawModeMenu.selectedIndex == 0 || (triangle_idx != 3 && draw_circle_step != 1)) {			
			...
			gl.bindBuffer(gl.ARRAY_BUFFER, vBuffer);
			gl.bufferSubData(gl.ARRAY_BUFFER, sizeof['vec3'] * index, flatten(getCircleVertices(center, r, 48)));
...
function render() {
	...
	// why have I not do simpler, do write the problem carefully, make the situation clear, the problem is not presented to you as before as clearly and it may disguide you
