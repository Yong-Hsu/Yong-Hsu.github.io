...
function init() {
	...
	var render = function () {
		...
		var V2 = lookAt(
			add(q_rot.apply(vec3(0, 0, -180)), at), 
			at, 
			q_rot.apply(up)
		);
...
function initEventHandlers(canvas, q_rot, q_inc) {
	...
	canvas.onmousemove = function (ev) { // Mouse is moved
		...
		if (dragging) {
			...
			u = v; 
			// why did I fail to make this logic
			// keep focusing on the q_rot part, I did notice have to make this keep updating the states or storing the previous step (unlikely, making it too complex)
			// so I should keep updating it, and this should be happening here where I have overlooked for making a change
