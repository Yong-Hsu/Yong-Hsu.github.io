...
var center = vec3(0.0, 2.0, -3.0);
var radius = 0.5;
var theta = 1.0; var beta = 0.0;
...
function init() {
	...
	*/
	...
	var lightProjection = perspective(90, 1, 1, 10); // perspective projection matrix
...
function render(model, program, programQuad) {
	...
	// recall
	// alpha =1, blend with the background
	// the problem of following the logic in the slides
	// the misplaced teapot, the texcoords were not right
	// set gl_FragColor.a = 1.0; for debugging
	// set gl_FragColor = vec4(gl_FragCoord.z, 0.0, 0.0, 1.0);
